...
from ..auth import ensure_ai_allowed, require_staff, require_teacher
...
def get_task(db: Session, task_id: int, user: User | None = None) -> WorksheetTask:
    task = db.get(WorksheetTask, task_id)
    if not task:
        raise HTTPException(404, "任务不存在")
    if user is not None and user.role == "teacher" and task.created_by != user.id:
        raise HTTPException(403, "无权操作他人创建的练习任务")
    return task
...
def create_tasks(body: WorksheetTaskCreate, db: Session = Depends(get_db),
    ...
    for sid in student_ids:
        ...
        # 校验来源记录必须属于该学生且出自本人（课程/作业），不能混入他人信息
        bad_fb = [i for i in fb_ids if not db.query(CourseFeedback).join(
            Course, CourseFeedback.course_id == Course.id).filter(
            CourseFeedback.id == i, Course.student_id == sid,
            Course.teacher_id == teacher.id).first()]
        if bad_fb:
            raise HTTPException(400, f"课程反馈 {bad_fb} 不属于所选学生或非本人课程，请重新选择")
        bad_sub = [i for i in ids if not db.query(Submission).join(
            Assignment, Submission.assignment_id == Assignment.id).filter(
            Submission.id == i, Submission.student_id == sid,
            Assignment.created_by == teacher.id).first()]
        if bad_sub:
            raise HTTPException(400, f"作业批改记录 {bad_sub} 不属于所选学生或非本人作业，请重新选择")
...
def list_tasks(status: str = "", limit: int = 50, db: Session = Depends(get_db),
    ...
    if user.role == "teacher":
        q = q.filter(WorksheetTask.created_by == user.id)  # 教师之间数据隔离
    if status:
...
def get_settings(_: User = Depends(require_staff)):
